...
`couponmoa-scheduler`는 **쿠폰 사용 통계 집계, 쿠폰 상태 갱신, 쿠폰 만료 알림 등 주기적으로 실행되는 작업을 담당하는 배치/스케줄링 전용 서버**입니다.

Spring Batch와 Spring Scheduler를 활용하여 안정적이고 자동화된 작업 처리를 목표로 합니다.
...
- 대량 데이터를 효율적으로 처리하기 위해 **Spring Batch + 파티셔닝** 사용
...
- **매일 자정**, 쿠폰 서버에 만료 알림 요청
- 쿠폰 서버가 만료 하루 전 쿠폰 조회 -> 사용자 서버가 알림 대상 조회 → 알림 서버가 이메일 전송
